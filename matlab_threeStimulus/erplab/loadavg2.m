...
%
% Modified by Javier Lopez-Calderon. New outputs (acceptcnt, rejectcnt) and more... September 2012.
%

function [signal, variance, chan_names, pnts, rate, xmin, xmax, acceptcnt, rejectcnt] = loadavg2( FILENAME )
if nargin<1
      help loadavg2
      return;
end
...
%       [signal, variance, chan_names, pnts, rate, xmin, xmax, acceptcnt, rejectcnt] = loadavg_bcl( FILENAME );
%       signal = signal';
%       return;
% end
...
% ---------------------------------------------------------------------
fseek(fid, S_nsweeps_offset_total, 'bof'); % skip the firsts 362 from BOF 
...
if variance_flag
      for elec = 1:chan
            ...
            variance(:, elec) = fread(fid, pnts, 'float32')*calib(elec)/nsweeps; 
      end;
else
      variance = 'novariance';
end;
...
% 
% % ----------------------------------------------------------------------------------------------------------------------
% function [signal,chan_names,variance, pnts, rate, xmin, xmax, acceptcnt, rejectcnt] = loadavg_bcl(FILENAME,chanNameList)
...
%       %fseek(fid, 900, 'bof');	% skip 387 bytes
% 
%       % read electrode configuration
%       % ----------------------------
%       for elec = 1:chan
...
%       
%       for elec = 1:chan
%             % To scale a data point to microvolts, multiply by the channel-specific calibration factor
%             % (i.e., for electrode j:
%             % channel[j]->calib) and divide by the number of sweeps in the average (i.e., channel[j]->n);
%             % skip sweeps header and read data
%             % --------------------------------
%             fseek(fid, 5, 'cof');
