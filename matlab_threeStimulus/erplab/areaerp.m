...
function varargout = areaerp(data, fs,latsam, op, coi, frac, fracarearep)
...
fsn = 2000;     % new sample rate 10Khz
Tsn = 1/fsn;     % new sample period
Fk  = fsn/fs;    % oversampling factor

if Fk>1 % interpolate data
      nsam = length(data);
      aa = 1; bb = nsam;
      newsam   = linspace(aa,bb,Fk*(bb-aa+1));
      data     = spline(aa:bb, data(aa:bb),newsam); % interpoled data    
      latsam   = round(latsam*Fk)-1;
else
      Fk=1;
end
...
if isempty(a) || isempty(b)
      ...
      %  Calculates area
      %
      A = single(Tsn*trapz(data(a:b))); % April 21, 2011
end
if nargout==1
      varargout{1} = A;
elseif nargout==2 || nargout==3
      ...
      if A~=0
            ...
            if Ao~=0
                  ...
                  while plow < phigh                        
                        ...
                        Ax   = single(Tsn*trapz(data(a:pmid)));
      ...
      % Frac latency
      %
      varargout{2} = L/Fk; % rescale. samples
