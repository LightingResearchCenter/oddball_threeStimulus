function s = pre_remove5060hz_modified(s, HDR, MODE)
...
if strfind(upper(MODE),'PCA') % strfind(upper(MODE),'SVD')
	if flagNoSegmentSupport, 	
        ...
        B = real(poly(r*exp(1i*2*pi*NotchFreq/HDR.SampleRate*[1,-1])));
...
elseif ~isempty(strfind(upper(MODE),'FIT'))
	if flagNoSegmentSupport, 	
        ...
        u = exp(1i*2*pi*NotchFreq/HDR.SampleRate*[1:size(s,1)]');
        A = covm(u,s);
        s = s - 2*real(u*A);
...
elseif ~isempty(strfind(upper(MODE),'NOTCH'))
	if flagNoSegmentSupport, 	
        ...
        if ~isempty(strfind(upper(MODE),'+'))
                B = real(poly(exp(1i*2*pi/HDR.SampleRate*[NotchFreq,HDR.SampleRate-NotchFreq])));
        else
                warning('NOTCH+ - mode is not recommended');        
                B = real(poly(exp(1i*2*pi/HDR.SampleRate*[NotchFreq:2*NotchFreq:HDR.SampleRate-NotchFreq/2])));
        end;
