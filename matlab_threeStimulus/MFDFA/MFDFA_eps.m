function [Ht,Htbin,Ph,Dh] = MFDFA_eps(signal,scale,m,eps,Fig)
...
% eps:          eps threshld to reject "noise"/ too small values
%               (added by Petteri Teikari to MFDFA_2 based on the original
%               article, see page 15 and 2nd column for details of Ihlen (2012)
%
% OUTPUT VARIABLES---------------------------------------------------------
%
...
%preallocate
segments = zeros(1, length(scale0));
Fq0 = zeros(1, length(scale0));
RMS0 = cell(1, length(scale0));
...
for ns=1:length(scale),
    ...
    % RMS{ns}(RMS{ns} < eps) = NaN; % added by Petteri
    F(ns)=exp(0.5*mean(log(RMS{ns}.^2)));
end

%preallocate
% Ht = zeros(length(scale), ?)
...
if Fig==1
   ...
   Ht_mode=mean(Htbin(Ph==max(Ph))); % original   
